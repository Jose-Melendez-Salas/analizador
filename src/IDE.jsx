...
// Importaciones de lógica que no deben cambiarse:
...
const handleRunAnalysis = async (silent = false) => {
    ...
    try {
        // 1. Análisis léxico y sintáctico (ahora con Acorn)
        ...
        // Obtenemos el AST que nos devuelve Acorn
        const ast = lexSyntaxResults.ast;

        // 2. Análisis semántico (SOLO si el AST se generó correctamente)
        const semanticResults = analyzeSemantics(ast); // <--- LE PASAMOS EL AST
        setSemanticResult(semanticResults.result);

        // 3. Estadísticas (el resto del código sigue igual)
        const stats = {
...
`;
  ...
  const panels = [
    { id: "lexical", name: "Léxico", icon: Zap, color: "blue", result: lexicalResult },
    { id: "syntactic", name: "Sintáctico", icon: Search, color: "emerald", result: syntacticResult },
    { id: "semantic", name: "Semántico", icon: Brain, color: "purple", result: semanticResult }
  ];
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {panels.map(panel => {
            ...
            const Icon = panel.icon;
            return (
              <div key={panel.id} className="flex-1 flex flex-col p-6 overflow-hidden">
